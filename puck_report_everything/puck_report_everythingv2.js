...
function startup() {
  ...
  // I"m trying one or the other sensors, or both, by (un)commenting the lines below
  setupAcceleration();
  setupIllumination();
...
function setupIllumination(){
  // our illumination data, no matter what the input values are, will get scaled from 0-100, 
  // where 0 is the lowest illumination input it's ever received since starting up,
  // and 100 is the highest ever received since starting up.
  illumScale = new dynScale(false, false, 0, 100);
  // every illumReadMS ms, read the illumination data
  setInterval(readIllumination, illumReadMS);
  // every illumSendMS ms, send the illumination data
  setInterval(sendIllumination, illumSendMS);
}

function readIllumination(){
  let illumValue = Puck.light();
  // scale the value using dynScale, then round it
  illumValue = Math.round(illumScale.scale(illumValue));
  // this is the data package that should be readable as dynamb data when stringified
...
function sendIllumination(){
  ...
  queue.addToQueue("illum", illumPriority, illumDurationMS, "replace", 
    // this function gets called before the item is sent, and adds the illumPackage to the items data property
    // the illumPackage object was created in readIllumination
    function(){
      return illumPackage;
    }, 
    function(item){
      ...
      // the item.data property is the illumPackage object created in readIllumination.
...
function setupAcceleration(){
  ...
  setInterval(sendAcceleration, accelSendMS);
}
...
function sendAcceleration(){
  queue.addToQueue("accel", accelPriority, accelSendMS, "replace", 
    function(){
      return accelPackage;
    }, 
    function(item){
